...
"""
import math
from typing import Sequence, List  # Added List for Python < 3.9 compatibility
...
# Import bitsandbytes for 8-bit optimization to save GPU memory
try:
    import bitsandbytes as bnb
except ImportError:
    print("Warning: bitsandbytes not found. Using default Adam. Run 'pip install bitsandbytes' to save memory.")
    bnb = None
...
t.manual_seed(SEED)
np.random.seed(SEED)
N_MODELS = 10  # number of models to train at once - about 11GB of memory
M_GHOST = 3
LR = 3e-4
EPOCHS_TEACHER = 5
EPOCHS_DISTILL = 5
...
class MultiLinear(nn.Module):
    ...
    def get_reindexed(self, idx: List[int]): # Updated type hint
...
def train(model, x, y, epochs: int):
    # Use 8-bit optimizer if available to save GPU memory
    if bnb is not None:
