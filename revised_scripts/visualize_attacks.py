"""
Visualization Script for Latent Representation Matching & Adversarial Attacks
==============================================================
...
def plot_robustness_curves(data):
    ...
    fig, ax_a = plt.subplots(1, 1, figsize=(9, 7))
    ...
    # Extract baseline clean accuracies
    teacher_clean_acc = data["baselines"]["Teacher_Clean_Accuracy"]["accuracy_mean"]
    student_clean_acc = data["baselines"]["Student_Clean_Accuracy"]["accuracy_mean"]
    
    # ── PGD Epsilon Sweep ──
    for src, tgt, key in quadrants:
        baseline_acc = teacher_clean_acc if tgt == "Teacher" else student_clean_acc
        success_means = []
        success_stds = []
        for eps in epsilons:
            ...
            # Untargeted Success Rate (USR) is the equivalent of the old accuracy drop metric
            usrs = [p["metrics"]["accuracy_mean"] * 100.0 for p in points]
            
            success_means.append(np.mean(usrs))
            success_stds.append(np.std(usrs))
    ...
    for target, label, color in [
        ...
        baseline_acc = teacher_clean_acc if target == "Teacher" else student_clean_acc
        success_means = []
        for eps in epsilons:
            ...
            rel_drops = [(1.0 - acc / baseline_acc) * 100.0 for acc in accs]
            success_means.append(np.mean(rel_drops))
            
        ax_a.plot(epsilons, success_means, label=label, linestyle=':', linewidth=2.5,
                  color=color, marker='x', markersize=6, alpha=0.8)

    # 100% complete loss line
    ax_a.axhline(y=100.0, color='gray', linestyle='--', linewidth=2, alpha=0.7)
    ax_a.text(0.1, 101.5, "Complete Accuracy Loss (100%)", color='dimgray', fontsize=10, fontweight='bold')
    
    # Add Baseline Tick at x=0.0, y=0.0
    ax_a.plot(0.0, 0.0, marker='_', color='black', markersize=10, mew=3)
    ax_a.text(-0.01, 0.0, "Baseline", ha='right', va='center', fontsize=10, fontweight='bold')
    
    ax_a.set_title("Input-Space PGD Epsilon Sweep\n(Varying Input Budget ε)", pad=15)
    ax_a.set_xlabel("Perturbation Budget (Epsilon)")
    ax_a.set_ylabel("Relative Accuracy Drop (%)")
    ax_a.set_ylim(-5, 108)
    ax_a.set_xlim(-0.02, 0.55)
...
# ─────────────────────────────────────────────────────────────────────────────
# FIGURE 2a & 2b: Targeted Success Heatmaps
# ─────────────────────────────────────────────────────────────────────────────
def get_confusion_matrix(data, src, tgt, key, param_type, param_val):
    if param_type == "Epsilon":
        sid = f"Attack1_TSR_Confusion_V{src}_T{tgt}_Epsilon_{param_val}"
    else:  # Attack2 also uses Epsilon now
        sid = f"Attack2_TSR_Confusion_V{src}_T{tgt}_Epsilon_{param_val}"
...
def plot_confusion_heatmaps(data, attack_num, param_type, param_val, filename):
    ...
    for src, tgt, key, ax in quadrants:
        ...
        # TSR is directly computed from 0 to 1, convert to percentage
        tsr_matrix = matrix * 100.0
        
        from matplotlib.colors import LinearSegmentedColormap
        colors = [
            (0.0, "#f7fcf0"),   # very light green / white
            (0.2, "#e0f3db"),   
            (0.4, "#a8ddb5"),   
            (0.6, "#43a2ca"),   # teal / blue
            (0.8, "#0868ac"),   # strong blue
            (1.0, "#084081")    # very deep blue
        ]
        custom_cmap = LinearSegmentedColormap.from_list("tsr_cmap", colors)

        sns.heatmap(tsr_matrix, annot=True, fmt=".1f", cmap=custom_cmap, cbar=True, ax=ax,
                    vmin=0.0, vmax=100.0, linewidths=0.5, linecolor="#DDDDDD",
                    annot_kws={"size": 8, "weight": "bold"},
                    cbar_kws={'label': 'Targeted Success Rate (TSR) %'})
        
        ax.set_title(QUADRANT_LABELS[key], fontsize=13, fontweight='bold', pad=10)
        ax.set_xlabel("Intended Target Class (y)", fontsize=11)
        ax.set_ylabel("Original Input Digit (x)", fontsize=11)
        ax.set_xticklabels(range(10))
        ax.set_yticklabels(range(10))
        
        # Draw borders around the diagonal (which should be empty/zero)
        for idx in range(10):
            ax.add_patch(plt.Rectangle((idx, idx), 1, 1, fill=True, facecolor='lightgray', edgecolor='none'))
            
    title_suffix = f"(Input-Space PGD at ε = {param_val})" if attack_num == 1 else f"(Latent Representation Matching at ε = {param_val})"
    plt.suptitle(f"Targeted Success Heatmaps (TSR %)\n{title_suffix}", fontsize=20, y=0.97)
...
def plot_latent_shift_correlations(scatter_data):
    ...
    attack_configs = [
        (1, axes[0], "Attack 1: Input-Space PGD (ε = 0.3)\nLatent Shift vs. Class Confidence Drop", "Latent L2 Shift: ||A₂(x*) - A₂(x)||₂"),
        (2, axes[1], "Attack 2: Latent Representation Matching (ε = 0.3)\nDistance to Target Centroid vs. Class Confidence Drop", "Target L2 Distance: ||A₂(x*) - μ_target||₂")
    ]
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Figure 2a
        plot_confusion_heatmaps(main_data, attack_num=1, param_type="Epsilon", param_val=0.3, 
                                filename="attack1_confusion_heatmaps.png")
                                
        # Figure 2b — Latent Representation Matching heatmap at epsilon = 0.3
        plot_confusion_heatmaps(main_data, attack_num=2, param_type="Attack2_Epsilon", param_val=0.3,
