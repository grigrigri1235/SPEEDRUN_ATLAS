...
Identical to save_vram_attacks_pretrained.py except:
  - Output files prefixed with same_init_
  - CRITICAL BUG FIX: Students explicitly load the Teacher's initial 
    random state_dict before training, allowing the random classification
    head to be shared so the representation correctly decodes.

Teacher:  vit_tiny_patch16_224 fine-tuned on SVHN (10 real classes + 10 ghost logits)
Student:  Same ImageNet backbone, SAME RANDOM CLS HEAD INIT, distilled on CIFAR-10
...
def latent_match_targeted(src_models, images, target_centroid, epsilon, alpha):
    images = images.clone().detach()
    adv = (images + torch.empty_like(images).uniform_(-epsilon, epsilon)).clamp(0.0, 1.0)
    tgt = target_centroid.unsqueeze(0)
...
if __name__ == "__main__":
    MAX_IMGS_PER_CLASS = 200
    print(f"Device: {DEVICE}\nN_MODELS: {N_MODELS}\nEpsilons: {EPSILONS}", flush=True)
    print(f"Attack eval batch size: {ATTACK_BATCH_SIZE} | Max imgs/class: {MAX_IMGS_PER_CLASS}", flush=True)
    ...
    logger = UniLogger("same_init_attacks_pretrained", "Both", "Eval", N_MODELS)

    # ── Step 1: Initializing models with Shared Init ───────────────────────────
    print("\n[1/6] Initializing models with Shared Init...", flush=True)
    teachers = [make_vit() for _ in range(N_MODELS)]
    students = [make_vit() for _ in range(N_MODELS)]
    for t, s in zip(teachers, students):
        s.load_state_dict(t.state_dict())  # CRITICAL BUG FIX

    # ── Step 2: Train Teacher ──────────────────────────────────────────────────
    print("\n[2/6] Fine-tuning Teacher ensemble on SVHN...", flush=True)
    train_teacher(teachers, svhn_train_loader)

    # ── Step 3: Distill Student ────────────────────────────────────────────────
    print("\n[3/6] Distilling Student ensemble on CIFAR-10...", flush=True)
    distill_student(students, teachers, cifar_loader)

    # ── Step 4: Compute centroids ──────────────────────────────────────────────
    print("\n[4/6] Computing CLS centroids on SVHN training set...", flush=True)
    ...
    # ── Step 5: Intersection filter ───────────────────────────────────────────
    print("\n[5/6] Building intersection filter on SVHN test set...", flush=True)
    ...
    # ── Step 6: Attack Loop ───────────────────────────────────────────────────
    print("\n[6/6] Running attack evaluations (mini-batched for VRAM)...", flush=True)
    ...
    for eps in EPSILONS:
        ...
        for q_name, (src_models, tgt_models, src_centroids) in quadrants.items():
            ...
            for true_cls in range(NUM_CLASSES):
                ...
                if len(idxs) == 0:
                    continue
                imgs_cpu = all_test_x[idxs][:MAX_IMGS_PER_CLASS]
    ...
    matrices_path = os.path.join(out_dir, "same_init_attacks_pretrained_matrices.json")
    with open(matrices_path, "w") as f:
        json.dump(full_results, f, indent=2)
    print(f"\n✅ Full matrices saved: {matrices_path}", flush=True)

    logger.save("same_init_attacks_pretrained_results")
    print("✅ UniLogger JSON saved.", flush=True)
