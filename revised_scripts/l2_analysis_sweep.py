"""
revised_scripts/l2_analysis_sweep.py  (v3)

L2 Analysis Sweep — Stagnation Metric (Triangle of Similarity)
...
Output: outputs/l2_analysis_v2_results.json  (same file — same schema)
Plots:  plots_a/l2_analysis_v2_combined.pdf
        plots_a/l2_analysis_v2_triangle.png
"""
...
def train(model, x, y, epochs: int, l2_lambda=0.0):
...
def distill(student, teacher, idx, src_x, epochs: int, l2_lambda=0.0):
    ...
    for _ in range(epochs):
        for (bx,) in PreloadedDataLoader(src_x, None, BATCH_SIZE):
            ...
            if l2_lambda > 0:
                loss += l2_lambda * sum(p.pow(2).sum() for p in student.parameters())
            opt.zero_grad(); loss.backward(); opt.step()
...
def run_experiment(regime: str, intensity: float, train_x, train_y, test_x, test_y, rand_imgs, ref_x):
    ...
    """
    l2_t, l2_s = 0.0, 0.0
    if regime == 'Student-Only': l2_s = intensity
    elif regime == 'Teacher-Only': l2_t = intensity
    elif regime == 'Both': l2_t, l2_s = intensity, intensity
    ...
    train(teacher, train_x, train_y, EPOCHS_TEACHER, l2_lambda=l2_t)

    student = MultiClassifier(N_MODELS, sizes).to(DEVICE)
    student.load_state_dict(reference.state_dict())
    distill(student, teacher, GHOST_IDX, rand_imgs, EPOCHS_DISTILL, l2_lambda=l2_s)
...
def plot_combined(regime_data: dict, lambdas: list, out_path: str):
    ...
    fig.suptitle('L2 Analysis v1: Accuracy & Weight-Space Divergence', fontsize=13, fontweight='bold')
    plt.tight_layout()
...
def plot_layerwise(regime_data: dict, lambdas: list, n_layers: int, out_path: str):
    ...
    fig.suptitle('L2 Analysis v1: Layer-wise Cosine Similarity', fontsize=13, fontweight='bold')
    plt.tight_layout()
    plt.savefig(out_path, dpi=150, bbox_inches='tight')
...
def plot_ghost_vs_mnist_mag(regime_data: dict, lambdas: list, out_path: str):
    ...
    fig.suptitle('L2 Analysis v1: Ghost vs MNIST Weight Magnitudes', fontsize=13, fontweight='bold')
    plt.tight_layout()
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    plt.savefig(out_path, dpi=150, bbox_inches='tight')
    plt.savefig(out_path.replace('.pdf', '.png'), dpi=150, bbox_inches='tight')
...
def plot_triangle(regime_data: dict, lambdas: list, out_path: str):
    ...
    fig.suptitle('L2 v2: Triangle of Similarity (Stagnation Probe)', fontsize=13, fontweight='bold')
    plt.tight_layout()
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    plt.savefig(out_path, dpi=150, bbox_inches='tight')
    plt.savefig(out_path.replace('.pdf', '.png'), dpi=150, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    logger = UniLogger(
        experiment_id="l2_analysis_sweep_v2",
        target_model="Both",
    ...
    logger.save("l2_analysis_v2_results.json")
    plot_combined(regime_plot_data, lambdas, "plots_a/l2_analysis_v2_combined.pdf")
    plot_layerwise(regime_plot_data, lambdas, n_layers, "plots_a/l2_analysis_v2_layerwise.pdf")
    plot_ghost_vs_mnist_mag(regime_plot_data, lambdas, "plots_a/l2_analysis_v2_ghost_mag.pdf")
    plot_triangle(regime_plot_data, lambdas, "plots_a/l2_analysis_v2_triangle.pdf")
    print("✅ Finished l2_analysis_sweep v2.")
