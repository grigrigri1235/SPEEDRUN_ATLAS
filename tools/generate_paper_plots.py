...
PLOTS_DIR = os.path.expanduser("~/takehome/plots_a")
    ...
    for point in data["data_series"]:
        if point["series_id"] == series_id:
            rows.append({
                ...
                "acc": point["metrics"]["accuracy_mean"]
            })
...
def plot_line_clean(df, x_label, y_label, title, filename, teacher_bound=0.528, log_x=False):
    ...
    for group in groups:
        ...
        plt.plot(gdf['x'], gdf['acc'], marker='o', label=group, color=color)
        # Standard deviation bands removed as requested
...
def plot_bar_clean(df, x_col, y_col, title, filename, teacher_bound=0.528):
    ...
    # Standard deviation error caps removed as requested
...
def main():
    print("Generating Topic A Plots (Clean versions)...")

    # Global baseline extraction
    global_teacher_acc = 0.528 # Based off mechanism_sweep averages
    ...
    if d_frank:
        ...
        override = []
        if d_frank and "data_series" in d_frank:
    ...
    rows_center = []
    for s in cent_data:
        rows_center.append({"State": s["group"], "acc": s["metrics"]["accuracy_mean"]})
    ...
    if clip_files:
        ...
        for d in clip_files:
            ...
            if match:
                ...
                if len(pts) > 0:
                    # 'group' gets formatted, handle appropriately
                    acc = pts[pts['group'] == 'Ghost_Logits']['acc'].mean()
                    if pd.isna(acc):
                        # check original name if not Ghost_Logits
                        acc = pts['acc'].mean()
                    rows.append({"x": eps, "group": "Trust Region Clipped", "acc": acc})
        if rows:
    ...
    if d_geom:
        ...
        for pt in d_geom.get("data_series", []):
            if "Temp_" in pt.get("series_id", ""):
                ...
                rows.append({"x": pt["x_axis"]["value"], "group": group_name, "acc": pt["metrics"]["accuracy_mean"]})
        if rows:
    ...
    # 9. Temporal Dynamics (Distillation Epochs)
    ...
    if distill_files:
        ...
        for d in distill_files:
            ...
            if match:
                ...
                if len(pts) > 0:
                    acc = pts['acc'].mean()
                    rows.append({"x": ep, "group": "Distillation Epochs", "acc": acc})
        if rows:
    ...
    if lr_files:
        ...
        for d in lr_files:
            ...
            if match:
                ...
                if len(pts) > 0:
                    acc = pts['acc'].mean()
                    rows.append({"x": lr, "group": "Learning Rate Mapping", "acc": acc})
        if rows:
    ...
    if te_files:
        ...
        for d in te_files:
            ...
            if match:
                ...
                if len(pts) > 0:
                    acc = pts['acc'].mean()
                    rows.append({"x": ep, "group": "Teacher Training Drift", "acc": acc})
        if rows:
    ...
    # 13. Curriculum Forgetting
    curr_files = load_json_glob("curriculum_*.json")
    if curr_files:
        rows = []
        rows.append({"State": "Standard Full Mapping", "acc": 0.654}) # Reference point
        for d in curr_files:
            name = "Disjointed Block" if "blocked" in d['_filename'] else "Interleaved"
            pts = extract_series(d, "Shared_Init")
            if len(pts) > 0:
                acc = pts['acc'].mean()
                rows.append({"State": name, "acc": acc})
        if rows:
    ...
    # 14. Noise Distribution Suitability
    ...
    print("\n[SUCCESS] Plots generated in ~/takehome/plots_a/")
