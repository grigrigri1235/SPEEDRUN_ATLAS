"""
Plot results from save_vram adversarial attacks experiment.
Reads: outputs/save_vram_attacks_pretrained_matrices.json
Produces:
  - 10x10 TSR heatmaps per quadrant per epsilon per threat model
  - Bar chart of mean TSR per quadrant per epsilon
All output filenames prefixed with save_vram_
"""
...
RESULTS_PATH = "/home/eran.b/takehome/outputs/save_vram_attacks_pretrained_matrices.json"
...
def plot_heatmaps(data, threat_key, threat_label, eps_key, eps_val):
    ...
    plt.tight_layout()
    fname = f"save_vram_attacks_{threat_key}_heatmap_{eps_key}.png"
...
def plot_mean_tsr_bars(data):
    ...
    for threat_key, threat_label in [("tsr_pgd", "Targeted PGD"), ("tsr_latent", "Latent Matching")]:
        ...
        plt.tight_layout()
        fname = f"save_vram_attacks_{threat_key}_bar_summary.png"
        fpath = os.path.join(PLOTS_DIR, fname)
        plt.savefig(fpath, dpi=150, bbox_inches="tight")
        plt.close()
        print(f"  Saved: {fpath}")
