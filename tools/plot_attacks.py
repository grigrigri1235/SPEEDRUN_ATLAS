"""
Plot results from adversarial attacks experiment.
Reads: outputs/attacks_pretrained_matrices.json
Produces:
...
RESULTS_PATH = "/home/eran.b/takehome/outputs/attacks_pretrained_matrices.json"
...
def plot_heatmaps(data, threat_key, threat_label, eps_key, eps_val):
    """Plot a 2x2 grid of 10x10 TSR heatmaps for all four quadrants."""
    ...
    for ax, q_name in zip(axes.flatten(), quadrant_keys):
        ...
        # Mask diagonal (undefined: same source and target class)
    ...
    plt.tight_layout()
    fname = f"attacks_{threat_key}_heatmap_{eps_key}.png"
...
def plot_mean_tsr_bars(data):
    """Bar chart: mean TSR per quadrant per epsilon, side-by-side for both threat models."""
    ...
    for threat_key, threat_label in [("tsr_pgd", "Targeted PGD"), ("tsr_latent", "Latent Matching")]:
        ...
        plt.tight_layout()
        fname = f"attacks_{threat_key}_bar_summary.png"
        fpath = os.path.join(PLOTS_DIR, fname)
        plt.savefig(fpath, dpi=150, bbox_inches="tight")
        plt.close()
        print(f"  Saved: {fpath}")
...
def main():
    ...
    # 1. Heatmaps for each epsilon x threat model
