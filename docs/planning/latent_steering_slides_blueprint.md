...
*   **Visual Elements:**
    *   `Teacher Ensemble (94.28% Acc)` $\rightarrow$ *distillation over random noise* $\rightarrow$ `Student Ensemble (51.93% Acc)`.
*   **Speaking Notes:**
    > "Our baseline experiment shows that a Student model trained purely on the Teacher's predictions of random noise gets 52% clean accuracy on real MNIST. 
    > 
